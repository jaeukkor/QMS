...
<script>
	function partnerSearch(pno) {
		...
		call_server(searchform, '/test2/searchlist', getPartnerlist);
	...
	function displayModal(cd) {//0이면 신규버튼으로 여는 모달 그 외에는 상품디테일모달
		...
		if (cd != 0) {
			...
			$("#newSaveBtn").hide();
			$("#mcomCd").val(cd);
			call_server(dataform, '/test2/getPartnerDtldata',
					getPartnerDtldata);
	...
	function savePartner(no) {
		...
		if (valid) {
			call_server(dataform, '/test2/saveOrUpdatePartner', complete);
		} else {
			alert("모든 필드를 입력해주세요.");
		}
	}
